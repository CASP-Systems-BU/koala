...
# fmt: off
KOALA_PATH = result_db_path("nexmark_query6_modified_lazy")
SAR_PATH = result_db_path("nexmark_query6_modified_SAR")
DRRS_PATH = result_db_path("nexmark_query6_modified_DRRS")
...
OFFSET = 0                     # shift x-axis left to hide warm-up ramp
...
fig, axes = plt.subplots(2, 3, figsize=(10,3.68), sharey='row')
...
for i in range(2):
    for j in range(3):
        ...
        if i == 0:
            ax.set_ylim([-(1.8e6*0.013),1.8e6])
        if i == 1:
            ax.set_ylim([-1,80])
            ax.text(0.97, 0.95, RATIO_LABELS[j], transform=ax.transAxes,
                    ha='right', va='top', color='black', fontsize=RATIO_FONTSIZE)

axes[1,0].set_xlabel("Time(s)", fontsize=17)
axes[1,1].set_xlabel("Time(s)", fontsize=17)
axes[1,2].set_xlabel("Time(s)", fontsize=17)
...
fig.subplots_adjust(left=0.09, right=0.971, top=0.93, bottom=0.17,
                    wspace=0.14, hspace=0.3)

plt.savefig("q6mod.pdf", dpi=600)
# plt.show()