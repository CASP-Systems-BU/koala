...
# fmt: off
KOALA_PATH = result_db_path("twitch_lazy")
REMOTE_PATH = result_db_path("twitch_Remote")
TIMES_SERIES, VALUES_SERIES_LA = export_metrics(DBS=[KOALA_PATH,REMOTE_PATH],
    TARGET_OPERATOR="viewer-Loyalty-Calculator",
...
fig, ax = plt.subplots(1, 1, figsize=(3.74, 2.2))
...
# fig.tight_layout()
fig.subplots_adjust(left=0.158, right=0.95, top=0.95, bottom=0.28, hspace=0.4)

plt.savefig("twitch_latency.pdf", dpi=600)
# plt.show()