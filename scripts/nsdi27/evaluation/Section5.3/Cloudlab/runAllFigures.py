...
"""
Regenerate every figure in Section5.3 (Cloudlab).
...
the `plt.savefig("Figure9.pdf")` calls land next to the script that produced
...
Handles both evaluation layouts, so this file can be dropped into any section:
  - flat, as here:      figure9.py, ... alongside util.py
...
Usage:
    ...
    python3 runAllFigures.py --only figure9     # substring match on script name
...
def label_for(figureDir: str, scriptName: str) -> str:
    """'Figure6/evaluateAzure.py' when nested, plain 'figure9.py' when flat."""
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Regenerate every figure in Section5.3 (Cloudlab)"
    )
    parser.add_argument(
        "--only",
        default="",
        help="Run only scripts whose figure dir or file name contains this "
        "substring (case-insensitive), e.g. figure9",
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
    ...
    print("=" * 64)
    print(f"  Section5.3 (Cloudlab) figures: {len(scripts)} script(s)")
