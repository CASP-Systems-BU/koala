...
# directory: Figure8/ -> Section5.2/ -> evaluation/ -> nsdi27/ -> scripts/
...
def latency_averaged(
    db_path: str, operator_prefix: str
) -> tuple[list[int], list[float]]:
...
def trim_to_shortest(
    times: Iterable[int], values: Iterable[float]
) -> tuple[list[int], list[float]]:
    """Truncate a (times, values) pair to their common length.

    Metrics collected from different operators do not necessarily cover the same
    number of timestamps - e.g. OutputRate at "source" vs NumBytesTransferred at
    "statefulMapper" - so a value series taken from one operator cannot be
    plotted against timestamps taken from another without trimming first. Note
    that sliding_avg() zips its two arguments and would silently return the
    shorter length, deferring the error to the plot call.
    """
    t = list(times)
    v = list(values)
    n = min(len(t), len(v))
...
def throughput_per_task(
    db_path: str, operator_prefix: str, metric_type: str = OUTPUT_RATE
) -> tuple[dict[int, list[int]], dict[int, list[float]]]:
    """Per-task output rate: one series per task of the operator, not aggregated.

    Needed for the skew figure, which shows that one task handles most of the
    hot keys before the rebalance. Unlike the aggregated helpers above, the
    return value is keyed by worker id: {workerId: timestamps}, {workerId: values}.
    """
    tm, vm = load_metric_by_operator(metric_type, operator_prefix, db_path)
    return (
        {w: list(ts) for w, ts in tm.items()},
        {w: _floats_to_python(vals) for w, vals in vm.items()},
    )
...
def export_metrics(DBS: list[str], TARGET_OPERATOR: str, FIGURE: str) -> tuple[list[int], list[float]]:
    ...
    if FIGURE == "Aggregate throughput at source":
        ...
        for p in DBS:
            ...
            vv.append(v)
        return (tt, vv)
    elif FIGURE == "Average processing time":
        tt = []
        vv = []
        for p in DBS:
            t, v = latency_averaged(p, TARGET_OPERATOR)
            tt.append(t)
            vv.append(v)
        return (tt, vv)
    elif FIGURE == "Per-task throughput":
        # One series per task instead of one aggregated series, so the outer
        # list is still indexed by DB but each entry is keyed by worker id
        tt = []
        vv = []
        for p in DBS:
            t, v = throughput_per_task(p, TARGET_OPERATOR)
            tt.append(t)
            vv.append(v)
        return (tt, vv)
    else:
