...
"""
Regenerate every figure in Section5.3 (AWS).
...
the `plt.savefig("Figure11.pdf")` calls land next to the script that produced
...
Handles both evaluation layouts, so this file can be dropped into any section:
  - flat, as here:      figure10.py, figure11.py, ... alongside util.py
...
Usage:
    ...
    python3 runAllFigures.py --only figure12    # substring match on script name
...
def label_for(figureDir: str, scriptName: str) -> str:
    """'Figure6/evaluateAzure.py' when nested, plain 'figure12.py' when flat."""
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Regenerate every figure in Section5.3 (AWS)"
    )
    parser.add_argument(
        "--only",
        default="",
        help="Run only scripts whose figure dir or file name contains this "
        "substring (case-insensitive), e.g. figure12",
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
    ...
    print("=" * 64)
    print(f"  Section5.3 (AWS) figures: {len(scripts)} script(s)")
