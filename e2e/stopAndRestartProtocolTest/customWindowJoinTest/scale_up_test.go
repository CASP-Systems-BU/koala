...
// Number of periods discussed above. Note that this config should be large
// enough to make rescaling work under lazy protocol. If the source1 finishes
// sending all data before rescaling, watermark won't proceed properly after
// rescaling.
const Num_Period = 6000

// Map for storing results for checking correctness
// map[period][key] = [sum1, sum2]
var results = make(map[int]map[int][2]int)

func TestCustomWindowJoinStopAndRestart(t *testing.T) {
	...
	client, workers, coordinator := testutils.DeployJob(
		numWorkers,
		func() *dataflow.Dataflow { return query(1, 0) },
		config,
	)
...
func checkCorrectness(
	...
	if newWorker != nil {
		// Get iterator for new worker state service
		...
		for iter.First(); iter.Valid(); iter.Next() {
			...
			if workerId != newWorker.WorkerId || key != -1 {
				t.Errorf(
					"Unexpected key in new worker state: key %d, workerId %d",
					key,
					workerId,
				)
			} else {
				log.Printf("New worker state key: key %d\n", key)
			}
		}
	}
}

func query(joinParallelism int, slowSourceWaitTimeSec int) *dataflow.Dataflow {
	...
	source1 := dataflow.NewSource[*tuple.Tuple3[int, int, int64]](
		...
		func(co collector.Collector) {
			...
			// Sleep to avoid finishing too early
			time.Sleep(time.Duration(slowSourceWaitTimeSec) * time.Second)
	...
	customWindowJoin.SetParallelism(joinParallelism)
