...
// Test CustomWindowJoin with ListState under lazy protocol with rescaling
...
func TestCustomWindowJoin(t *testing.T) {
	...
	if numNewWorkers != 1 {
		log.Fatalln(
			"There should be exactly 1 empty worker for scale up, found",
			numNewWorkers,
		)
	}
	...
	checkCorrectness(t, coordinator, newWorker)
