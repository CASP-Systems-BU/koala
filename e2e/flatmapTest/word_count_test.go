...
// In this test, we define a dataflow to count
// the number of occurrences of each word in a stream of sentences.
...
func wordCountQuery() *dataflow.Dataflow {
	...
	// Define flatmap
	flatmap := dataflow.NewFlatmap[*tuple.Tuple2[string, int64], *tuple.Tuple1[string]](
		"flatmap",
		func(t *tuple.Tuple2[string, int64], co collector.Collector) {
			words := strings.Split(t.V1, " ")
			for _, word := range words {
				co.Emit(&tuple.Tuple1[string]{
					V1: word,
				})
			}
		},
