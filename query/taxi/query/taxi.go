...
type TaxiConfig struct {
	...
	ProducerIPs                     []string
	CalculateCellIdParallelism      int
	MovingMedianTripTimeParallelism int
	SinkParallelism                 int
...
func (cfg *TaxiConfig) UnmarshalJSON(data []byte) error {

	// Start with the default configuration
	*cfg = TaxiConfig{
		ProducerIPs:                     []string{},
		CalculateCellIdParallelism:      1,
		MovingMedianTripTimeParallelism: 1,
		SinkParallelism:                 1,
		KafkaClusterIPs:                 []string{"localhost"},
		WatermarkInterval:               utils.Duration(100 * time.Millisecond),
		MaxAllowedDelay:                 utils.Duration(0),
		WindowDuration:                  utils.Duration(1800 * time.Second),
		ListLength:                      1000,
	}

	// Use alias to avoid infinite recursion
	type alias TaxiConfig
...
func Taxi(configFile string) *dataflow.Dataflow {
	...
	movingMedianTripTime := dataflow.NewStatefulMapper(
		...
		func(in *models.RouteInfo,
			...
			for i := len(mostRecentTrips); i < config.ListLength; i++ {
				dummyInput := dummyTaxiTrip[i%len(dummyTaxiTrip)]
			...
			validTrips := []*models.RouteInfo{}
			for i := 0; i < len(mostRecentTrips); i++ {
				if mostRecentTrips[i].V7 > 0 {
					validTrips = append(validTrips, mostRecentTrips[i])
				}
			}

			sort.Slice(validTrips, func(i, j int) bool {
				if validTrips[i].V9 == validTrips[j].V9 {
					return validTrips[i].V17 > validTrips[j].V17
				}
				return validTrips[i].V9 > validTrips[j].V9
			})

			n := len(validTrips)
			tripSecondMedian := validTrips[n/2]

			state.Update(mostRecentTrips[:config.ListLength])
			return &tuple.Tuple3[string, int64, int64]{
				V1: strconv.FormatInt(in.V17, 10),
				V2: tripSecondMedian.V9,
				V3: int64(n),
			}
		},
	)
