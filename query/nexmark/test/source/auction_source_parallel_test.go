...
// In this test, first we generate a given number of auction events, then use
...
var SourceResultAuction []models.AuctionEvent

func TestNewRandAuctionSourceMultiple(t *testing.T) {
	...
	testutils.DeployJob(
		numWorkers,
		newAuctionSourceMultipleQuery,
		configuration,
	)
	time.Sleep(10 * time.Second)
	sort.Slice(SourceResultAuction, func(i, j int) bool {
		return SourceResultAuction[i].V1 < SourceResultAuction[j].V1
	})
	log.Println("got", len(SourceResultAuction), "results")
	...
	for i, record := range SourceResultAuction {
...
func newAuctionSourceMultipleQuery() *dataflow.Dataflow {
	...
	sink := dataflow.NewSink(
		...
		func(t *models.AuctionEvent) {
			SourceResultAuction = append(SourceResultAuction, *t)
		},
	)
